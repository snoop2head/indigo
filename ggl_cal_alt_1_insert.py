...
def add_calendar_without_restriction():
    # make calendar artist name list based on calendar_alt collection
    calendar_col = db.calendar_alt.find()
    # print(artist_col)
    calendar_artist_name_list = []
    for artist_data in calendar_col:
        artist_name = artist_data['artist_name']
        calendar_artist_name_list.append(artist_name)
    ...
    for item in twentyfour_item_lists[0]:
        if item in calendar_artist_name_list:
            pass
        else:
